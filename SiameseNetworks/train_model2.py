...
for k in range(len(data_by_alph)):
    ...
    for epoch in range(conf['n_epochs']):
        ...
        ret = train_one_epoch(siamese_net, train_batches[str(k)], 
                              criterion, optimizer,                                               
                              epoch, conf['n_epochs'], avg_metrics=[accuracy_logits,],task=k, _prune=_prune)
        if ret is None:
            break
    ...
    for k2 in range(len(data_by_alph)):
        ret = train_one_epoch(siamese_net, train_batches[str(k2)], 
                              criterion, optimizer,                                               
                              epoch, conf['n_epochs'], avg_metrics=[accuracy_logits,],task=k2, _prune=_prune, stop_boole = True)
